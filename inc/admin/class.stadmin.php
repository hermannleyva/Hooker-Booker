    ...
    class STAdmin {
        ...
        function admin_enqueue_scripts($hook) {
            ...
            if ($google_api_key === 'on') {
                ...
                // wp_register_script('mapboxv52', 'https://api.tiles.mapbox.com/mapbox-gl-js/v1.0.0/mapbox-gl.js',array(),true,false);
                // wp_register_script('bt-mapboxv5-init2',get_template_directory_uri() . '/inc/plugins/ot-custom/fields/gmap/js/init-mapbox.js',array(),false,true);
                // wp_register_style('mapbox-css', 'https://api.tiles.mapbox.com/mapbox-gl-js/v1.0.0/mapbox-gl.css');
            }
            if (!in_array(get_post_type(), array('product', 'shop_order'))) {
                ...
                if (file_exists($lang_file)) {
                    wp_enqueue_script('select2-lang', get_template_directory_uri() . '/js/select2-new/js/i18n/' . $lang . '.js', ['jquery', 'select2.js'], null, true);
                } else {
                    $locale = TravelHelper::get_minify_locale($lang);
                    $lang_file = get_template_directory_uri() . '/js/select2-new/js/i18n/' . $locale . '.js';
                    if (file_exists($lang_file)) {
                        wp_enqueue_script('select2-lang', get_template_directory_uri() . '/js/select2-new/js/i18n/' . $locale . '.js', ['jquery', 'select2.js'], null, true);
                    }
                }
                wp_enqueue_style('st-select2', get_template_directory_uri() . '/js/select2-new/css/select2.css');
            }
            ...
            wp_register_script('fullcalendar', get_template_directory_uri() . '/js/fullcalendar-5.5.1/lib/main' . $prefix. '.js', ['jquery', 'moment'], NULL, TRUE);
            wp_register_script('fullcalendar-lang', get_template_directory_uri() . '/js/fullcalendar-5.5.1/lib/locales-all' . $prefix. '.js', ['jquery', 'fullcalendar'], NULL, TRUE);
            wp_register_style('fullcalendar-css', get_template_directory_uri() . '/js/fullcalendar-5.5.1/lib/main' . $prefix. '.css');
            ...
            wp_enqueue_script('st-custom-admin', get_template_directory_uri() . '/js/admin/custom-admin.js', ['jquery'], null, true);
            ...
            //use for solo home
            wp_register_script('jscolor.js', get_template_directory_uri() . '/v2/js/jscolor.js', ['jquery'], null, true);
            ...
            wp_localize_script('jquery', 'st_params', [
                ...
                'text_infant' => __('Infant: ', 'traveler'),
                ...
                'dateformat' => TravelHelper::getDateFormatJs(null, 'calendar'),
                'dateformat_convert' => TravelHelper::getDateFormatJs(null, 'admin-calendar'),
            ]);
        ...
        protected function verifyNonce($name = '_s', $action_name) {
            if (!isset($_POST[$name]) or ! wp_verify_nonce($_POST[$name], $action_name))
                return false;
            return true;
        }
