        ...
        class STHotel extends TravelerObject
        {
            ...
            public function st_filter_hotel_ajax()
            {
                ...
                if ( $style == '1' ) {
                    if ( $query->have_posts() ) {
                        ...
                        while ( $query->have_posts() ) {
                            $query->the_post();
                            echo st()->load_template( 'hotel/loop', 'list', [ 'taxonomy' => $jcategory ] );
                        }
                        echo "</ul>";
                    }
                } else {
                    ...
                    <div class="row row-wrap loop_hotel loop_grid_hotel style_box">
                        <?php
                            while ( $query->have_posts() ) {
                                $query->the_post();
                                echo st()->load_template( 'hotel/loop', 'grid', [ 'taxonomy' => $jcategory ] );
                            }
                        ?>
                    </div>
                    <?php
                ...
                if ( !$query->found_posts ) {
                    echo '<h3 class="ajax-filter-not-found">' . __( 'No hotel found', 'traveler' ) . '</h3>';
                }
            ...
            public function st_fetch_inventory()
            {
                ...
                if ( get_post_type( $post_id ) == 'st_hotel' ) {
                    $title_hotel = get_the_title($post_id);
            ...
            public function featch_dataroom( $hotel_id, $post_id, $post_name, $start, $end )
            {
                ...
                $post_id_calendar = TravelHelper::post_origin($post_id);
            ...
            function get_result_string()
            {
                ...
                if ( $query->found_posts ) {
                    if ( $query->found_posts > 1 ) {
                        $p1 = sprintf( __( '%s hotels', 'traveler' ), $query->found_posts );
                    } else {
                        $p1 = sprintf( __( '%s hotel', 'traveler' ), $query->found_posts );
                    }
                } else {
                    $p1 = __( 'No hotel found', 'traveler' );
                }
                ...
                if ( $start and $end ) {
                    $p3 = sprintf( __( 'on %s', 'traveler' ), date_i18n( 'M d', $start ) . ' - ' . date_i18n( 'M d', $end ) );
                }

                if ( $adult_num = STInput::get( 'adult_number' ) ) {
                    if ( $adult_num > 1 ) {
                        $p4 = sprintf( __( 'for %s adults', 'traveler' ), $adult_num );
                    } else {

                        $p4 = sprintf( __( 'for %s adult', 'traveler' ), $adult_num );
                    }
                ...
                if ( st_is_ajax() and STInput::post( 'room_search' ) ) {
                    ...
                    if ( $booking_period && $period < $booking_period ) {
                        $result = [
                            ...
                            'message' => sprintf( __( 'This hotel allow minimum booking is %d day(s)', 'traveler' ), $booking_period )
                        ];
                        echo json_encode( $result );
                        die;
                    }
                    if ( $date_diff < 1 ) {
                        $result = [
                            'status'    => 0,
                            'data'      => "",
                            'message'   => __( 'Make sure your check-out date is at least 1 day after check-in.', 'traveler' ),
                            'more-data' => $date_diff
                        ];
...
//				) ");
            ...
            public function _change_posts_fields( $fields )
            {
                ...
                $fields .= ', min(CAST(CASE WHEN IFNULL(tb.adult_price, 0) = 0 THEN tb.price ELSE tb.adult_price END AS DECIMAL) - (CAST(CASE WHEN IFNULL(tb.adult_price, 0) = 0 THEN tb.price ELSE tb.adult_price END AS DECIMAL) * ((IFNULL(tb3.discount_rate, 1))/100))) as st_price, tb2.price_avg as st_price_avg';
