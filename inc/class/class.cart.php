    ...
    class STCart {
        ...
        static function add_cart( $item_id, $number = 1, $price = false, $data = [] ) {
            ...
            if ( $st_is_woocommerce_checkout ) {
                ...
                if ( get_post_type( $item_id ) == 'st_flight' ) {
            ...
            if ( st()->get_option( 'booking_enable_captcha', 'on' ) == 'on' ) {
                ...
                if ( $allow_captcha == 'off' ) {
                    if ( ! $st_security_key ) {
                        return [
                            'status'  => false,
                            'message' => __( 'You did not enter the captcha', 'traveler' )
                        ];
                    }
                    ...
                    if ( ! $valid ) {
                        return [
                            'status'     => false,
                            'message'    => __( 'Captcha is not correct', 'traveler' ),
                ...
                <div class="checkbox <?php echo esc_attr( $name ); ?>">
                    <label>
                        <input class="i-check " name="create_account" type="checkbox"
                               value=" " <?php echo esc_attr( $checked . $disabled . $required ) ?> /><?php printf( __( 'Create %s account ', 'traveler' ), get_bloginfo( 'title' ) ) ?>
                ...
                <div class="checkbox <?php echo esc_attr( $name ); ?>">
                    <label>
                        <input class="i-check" value="1" name="term_condition"
                            ...
                            echo 'checked';
                        } ?>/><?php echo st_get_language( 'i_have_read_and_accept_the' ) . '<a target="_blank" href="' . get_the_permalink( st()->get_option( 'page_terms_conditions' ) ) . '"> ' . st_get_language( 'terms_and_conditions' ) . '</a> '. __('and', 'traveler') .' <a href="'. esc_url($page_privacy_policy_link) .'" target="_blank">'. __('Privacy Policy', 'traveler') .'</a>'; ?>
                    </label>
                </div>
            <?php }
        }
