    ...
    class STTour extends TravelerObject {
        ...
        public function st_filter_tour_ajax() {
            ...
            if (have_posts()) {
                while (have_posts()) {
                    the_post();
                    if ($st_style == '1') {
                        $content .= st()->load_template('tours/elements/loop/loop-1', null, array('tour_id' => get_the_ID(), 'taxonomy' => $jcategory));
                    }
                    if ($st_style == '2') {
                        $content .= st()->load_template('tours/elements/loop/loop-2', null, array('tour_id' => get_the_ID(), 'taxonomy' => $jcategory));
                    }
                }
                ob_start();
                if ($st_style == '1') {
                ...
                echo '<h3 class="ajax-filter-not-found">' . __('No tours found', 'traveler') . '</h3>';
            }
            ...
            ?>
            <p>
                <small><?php echo balanceTags($this->get_result_string()) ?>. &nbsp;&nbsp;
                    <?php
            ...
            <div class="row">
                <?php TravelHelper::paging($wp_query); ?>
            </div>
            <?php
            $ajax_filter_pag = ob_get_contents();
            ob_clean();
            ob_end_flush();
            $count = balanceTags($this->get_result_string());
            $result = array(
                ...
                $query .= ",CASE
                        ...
                        THEN
                           CASE
                                ...
                                THEN
                                        CAST(tb.price AS DECIMAL) - ( CAST(tb.price AS DECIMAL) / 100 ) * CAST(tb.discount AS DECIMAL)
        ...
        function get_result_string() {
            ...
            if ($query->found_posts) {
                if ($query->found_posts > 1) {
                    $p1 .= esc_html($query->found_posts) . __(' tours ', 'traveler');
                } else {
                    $p1 .= esc_html($query->found_posts) . __(' tour ', 'traveler');
                }
            } else {
                $p1 = __('No tour found', 'traveler');
            }
        ...
        function do_add_to_cart() {
            ...
            if ($adult_number + $child_number + $infant_number < $min_number) {
                STTemplate::set_message(sprintf(__('Min of people for this tour is %d people', 'traveler'), $min_number), 'danger');
                $pass_validate = FALSE;
            ...
            if (!st_validate_guest_name($tour_origin, $adult_number, $child_number, $infant_number) && empty($partner_create_booking)) {
