<?php
    ...
    if ( !class_exists( 'STActivity' ) ) {
        ...
        class STActivity extends TravelerObject
        {
            ...
            public function st_filter_activity_ajax(){
                ...
                if ( have_posts() ) {
                    while ( have_posts() ) {
                        the_post();
                        if($st_style == '1'){
                            $content .=st()->load_template('activity/elements/loop/loop-1',false,array("taxonomy"=>$jcategory));
                        }
                        if ( $st_style == '2' ) {
                            wp_enqueue_script( 'jquery.matchHeight-min' );
                            $content .=  st()->load_template('activity/elements/loop/loop-2',false,array("taxonomy"=>$jcategory));
                        }
                    }
	                if ( $st_style == '1' ) {
	                ...
	                echo '<h3 class="ajax-filter-not-found">' . __( 'No activity found', 'traveler' ) . '</h3>';
                }
            ...
            {
                register_sidebar( [
                    'name'          => __( 'Activity Search Sidebar 1', 'traveler' ),
                    'id'            => 'activity-sidebar',
                    'description'   => __( 'Widgets in this area will be shown on Activity', 'traveler' ),
                    'before_title'  => '<h4>',
                    'after_title'   => '</h4>',
                ...
                register_sidebar( [
                    'name'          => __( 'Activity Search Sidebar 2', 'traveler' ),
            ...
            function get_result_string()
            {
                ...
                if ( $query->found_posts ) {
                    if ( $query->found_posts > 1 ) {
                        $p1 = esc_html( $query->found_posts ) . __( ' activities ', 'traveler' );
                    } else {
                        $p1 = esc_html( $query->found_posts ) . __( ' activity ', 'traveler' );
                    }
                } else {
                    $p1 = __( 'No activity found', 'traveler' );
                }
            ...
            function do_add_to_cart()
            {
                ...
                if(!st_validate_guest_name($post_origin,$adult_number,$child_number,$infant_number) && empty($partner_create_booking))
                {
