...
if(!class_exists('ST_Woocommerce'))
{
    class ST_Woocommerce
    {
        ...
        function __construct()
        {
            ...
            add_filter( 'woocommerce_currency', array($this,'st_woocommerce_currency_symbol'), 1 );
        }

        /*Set currency woocommerce when checkout*/
        public function st_woocommerce_currency_symbol(  $currency ){
        ...
        public function setWoocommerceLocateTemplate($template, $template_name, $template_path){
            ...
            if(New_Layout_Helper::isNewLayout()){
                ...
                $template = locate_template('st_templates/layouts/modern/woo/' .$template_name);
                if (!is_file($template)) {
                    $template = $_template;
                }
            }
        ...
        function st_add_booking_fee(){
			...
			if ( $booking_fee == "on" ) {
				...
				$woocommerce->cart->add_fee( __('Fee', 'traveler'), $price_fee);
