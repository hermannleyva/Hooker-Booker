...
class TravelerObject
{
    ...
    {
        if($this->template_folder === 'st_flight'){
            $this->template_folder = 'flight';
        }
     ...
     *
     * */
    function get_near_by($post_id = false, $range = null, $limit = 5)
    {
        if (!$post_id)
            $post_id = get_the_ID();
        if (is_null($range)) {
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
    ...
    function getOrderby()
    {
        $this->orderby = [
            ...
            'avg_rate' => [
                'key' => 'avg_rate',
                'name' => esc_html__('Review', 'traveler')
            ]
        ];
    ...
    function _show_wc_cart_item_information_btn($cart_item_key = [])
    {
        ...
        print balancetags('<br><span data-hide = "' . __("Less", 'traveler') . ' <i class=&quot;fa fa-angle-up&quot;>" data-target= "#st_cart_item' . md5(json_encode($cart_item_key)) . '" data-toggle="collapse" class="_show_wc_cart_item_information_btn text-color booking-item-review-expand-more">' . __("More", 'traveler') . ' <i class="fa fa-angle-down"></i></span>');
    }
