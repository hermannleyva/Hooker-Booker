...
if (!class_exists("Single_Nav_Mainmenu")) {
    class Single_Nav_Mainmenu extends Walker_Nav_menu {
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            if ($args->walker->has_children) {
                $item_output = sprintf('%1$s<a%2$s >%3$s%4$s%5$s<i class="fa fa-angle-down"></i></a>%6$s', $args->before, $attributes, $args->link_before, apply_filters('the_title', $item->title, $item->ID), $args->link_after, $args->after
                );
            } else {
