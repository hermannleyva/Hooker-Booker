        ...
        class STReview
        {
            ...
            function like_review()
            {
                ...
                if ( $this->find_by( $comment_id ) ) {
                    ...
                    $data = [
                        'like_status' => true,
                        'message'     => __( 'You like this', 'traveler' ),
                    ...
                    echo json_encode( [
                        ...
                        'error'  => [
                            ...
                            'error_message' => __( 'Review does not exists', 'traveler' )
                        ]
                    ] );
                }
                exit();
            ...
            function save_comment_meta_data( $comment_id )
            {
                ...
                $comment_id_item = get_comment( $comment_id ); 
                $comment_post_id = $comment_id_item->comment_post_ID ;
                $current_post_type = get_post_type($comment_post_id);
                if(in_array($current_post_type,$all_postype)){
                    global $wpdb;
                    $array['comment_type'] = 'st_reviews';
