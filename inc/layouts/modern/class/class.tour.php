...
if (!class_exists('STTour')) {
    ...
    class STTour extends TravelerObject {
        ...
        public function st_format_tour_price(){
            ...
            <?php if ($item_id <= 0 || get_post_type($item_id) != 'st_tours') {

                wp_send_json(array('message' =>__('This tour is not available.', 'traveler'), 'price_from' => $price_from));
            ...
            if (!$adult_number and ! $child_number and ! $infant_number) {
                ...
                wp_send_json(array('message' =>__('Please select at least one person.', 'traveler'),'price_from' => $price_from));

            }

            if ($adult_number + $child_number + $infant_number < $min_number) {

                wp_send_json(array('message' =>sprintf(__('Min number of people for this tour is %d people', 'traveler'), $min_number), 'price_from' => $price_from));
            ...
            if ($max_number > 0) {
                ...
                if ($adult_number + $child_number + $infant_number > $max_number) {
                    ...
                    wp_send_json(array('message' =>sprintf(__('Max of people for this tour is %d people', 'traveler'), $max_number),'price_from' => $price_from));
            ...
            if (!$check_in || !$check_out) {
                ...
                wp_send_json(array('message' =>__('Select a day in the calendar.', 'traveler')));
            ...
            if ($tour_price_by != 'fixed_depart') {
                ...
                if ($compare < 0) {
                    ...
                    wp_send_json(array('message' =>__('This tour has expired', 'traveler')));
            ...
            if ($period < $booking_period) {
                ...
                wp_send_json(array('message' =>sprintf(__('This tour allow minimum booking is %d day(s)', 'traveler'), $booking_period),'price_from' => $price_from));
            ...
            if ($tour_price_by != 'fixed_depart') {
                ...
                if (!$tour_available) {
                    ...
                    wp_send_json(array('message' =>__('The check in, check out day is invalid or this tour not available.', 'traveler'),'price_from' => $price_from));
                ...
                if ($max_number > 0) {
                    ...
                    if ($free_people < ( $adult_number + $child_number + $infant_number )) {
                        ...
                        if (empty(trim($starttime))) {
                            ...
                            wp_send_json(array('message' =>sprintf(__('This tour is only available for %d people', 'traveler'), $free_people),'price_from' => $price_from));

                        } else {

                            wp_send_json(array('message' =>sprintf(__('This tour is only available for %d people at %s', 'traveler'), $free_people, $starttime),'price_from' => $price_from));
        ...
        public function st_filter_tour_ajax_location() {
            ...
            <span class="count-string">
                ...
                if ($query_service->found_posts):
                    ...
                    if ($last > $query_service->found_posts)

                        $last = $query_service->found_posts;

                    echo sprintf(__('%d - %d of %d ', 'traveler'), (int) $posts_per_page * ($page - 1) + 1, $last, $query_service->found_posts);

                    echo ( $query_service->found_posts == 1 ) ? __('Tour', 'traveler') : __('Tours', 'traveler');
        ...
        function add_sidebar() {
            ...
            register_sidebar([
                ...
                'name' => __('Tours Search Sidebar 1', 'traveler'),

                'id' => 'tours-sidebar',

                'description' => __('Widgets in this area will be shown on Tours', 'traveler'),
            ...
            register_sidebar([
                ...
                'name' => __('Tour Single Sidebar', 'traveler'),

                'id' => 'tour-single-sidebar',

                'description' => __('Widgets in this area will be shown on all tour.', 'traveler'),
        ...
        function _get_join_query($join) {
            ...
            $join .= " INNER JOIN {$table_postmeta} as tb_postmeta ON tb.post_id = tb_postmeta.post_id AND tb_postmeta.meta_key = 'discount_type' ";

            $join .= " INNER JOIN {$table_st_tours} as tb_st_tours ON {$wpdb->prefix}posts.ID = tb_st_tours.post_id";
        ...
        function _get_where_query($where) {
            ...
            if (isset($st_search_args['location_id']) && !empty($st_search_args['location_id'])) {
                ...
                $location_id = intval($st_search_args['location_id']);
        ...
        function _get_select_query($query) {
            ...
            if ($post_type == 'st_tours') {
                ...
                $query .= ", tb.min_price AS st_tour_price";
        ...
        function get_result_string() {
            ...
            if ($query->found_posts) {
                ...
                if ($query->found_posts > 1) {
                    ...
                    $p2 = sprintf(__('%s fishing trips found', 'traveler'), $query->found_posts);

                } else {

                    $p2 = sprintf(__('%s fishing trip found', 'traveler'), $query->found_posts);
                ...
                if ($max_number > 0) {
                    ...
                    if ($free_people < ( $adult_number + $child_number + $infant_number )) {
                        ...
                        if (empty(trim($starttime))) {
                            ...
                            STTemplate::set_message(sprintf(__('This tour is only available for %d people at %s', 'traveler'), $free_people, $starttime), 'danger');
