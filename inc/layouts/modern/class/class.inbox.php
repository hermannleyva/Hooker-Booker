...
if (!class_exists('ST_Inbox')) {
    class ST_Inbox
    {
        ...
        public function __sendMessageHandler()
        {
            if(!empty($_POST['st_send_message']) and !empty($_POST['item_id']))
            {
                ...
                $booking_data=[
                    ...
                    'drop-off'=>STInput::post('drop-off'),
                    ...
                    'location_name' => STInput::post('location_name'),
                ];
                if($inbox)
                {
                    // Update Old One
        ...
        function _send_message_partner(){
            ...
            $validator->set_rules( 'title', __( "Title", 'traveler' ), 'required|min_length[6]|max_length[100]' );
            $validator->set_rules( 'message', __( "Message", 'traveler' ), 'required|min_length[6]|max_length[200]' );
            ...
            if(!empty($message) && !empty($to_user)){
                ...
                if($send){
                    ...
                    $res['data'] = array(
                        ...
                        'created_at' => esc_html__('just now','traveler'),
                        'avatar' => st_get_profile_avatar(get_current_user_id(),50),
        ...
        function _check_inbox_notification(){
            ...
            if(!empty($count)){
                ...
                $res['message'] = sprintf(_n('You have %s new message', 'You have %s new messages', $count, 'traveler'), $count);
                $res['inbox_link'] = $url;
                if(isset($_SESSION['new_count']) && !empty($old_count = $_SESSION['new_count'])){
