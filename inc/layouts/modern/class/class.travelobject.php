...
class TravelerObject
{
    ...
    {
        echo balanceTags($this->load_view('wc_order_item_meta', null, [
            'item_id' => $item_id,
            'item' => $item,
            'order' => $order
        ]));
        echo balanceTags(st()->load_template('woo/cart-meta-deposit', null, [
            'item_id' => $item_id,
            'item' => $item,
            'order' => $order
        ]));
     ...
     *
     * */
    function get_near_by($post_id = false, $range = null, $limit = 4)
    {
        if (!$post_id)
            $post_id = get_the_ID();
        // Remove in ver 2.0
        /*if ( false !== ( $value = get_transient( 'st_items_nearby_' . $post_id . '_' . $range . '_' . $limit ) ) )
            return $value;*/
        if (is_null($range)) {
        $range = st()->get_option('distance_for_nearby', 25);
    }
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
    ...
    {
        if ($post_type == 'car_transfer' && st_check_service_available( 'st_cars' )) {
            ...
            if (empty($post_type) || !TravelHelper::checkTableDuplicate($post_type)) {
                return ['price_min' => 0, 'price_max' => 1000];
            }
            global $wpdb;
            ...
            if (array_key_exists('min_max_' . $post_type, self::$minMaxPrice)) {
                ...
                switch ($post_type) {
                    ...
                    case "st_tours":
                        ...
                        $where = STLocation::edit_where_wpml($where);
                        $sql = "SELECT
                                min(Convert(`min_price`, SIGNED)) as min,
                                max(Convert(`min_price`, SIGNED)) as max
                            FROM
                                {$wpdb->prefix}{$post_type}
                            {$join}
                            {$where}";
                            ...
                            CASE
                                WHEN {$wpdb->prefix}{$post_type}.adult_price != '0' and {$wpdb->prefix}{$post_type}.adult_price != ''
                                    ...
                                    CASE
                                        ...
                                        THEN
                                            CASE
                                            WHEN {$wpdb->prefix}{$post_type}.discount_type = 'amount'
                                            THEN
                                                CAST({$wpdb->prefix}{$post_type}.adult_price AS DECIMAL) - CAST({$wpdb->prefix}{$post_type}.discount AS DECIMAL)
                                            ELSE
                                    ...
                                    CASE
                                        ...
                                        THEN
                                            ...
                                            WHEN {$wpdb->prefix}{$post_type}.discount_type = 'amount'
                                            THEN
                                                CAST({$wpdb->prefix}{$post_type}.child_price AS DECIMAL) - CAST({$wpdb->prefix}{$post_type}.discount AS DECIMAL)
                                            ELSE
                                    ...
                                    CASE
                                        ...
                                        THEN
                                            ...
                                            WHEN {$wpdb->prefix}{$post_type}.discount_type = 'amount'
                                            THEN
                                                CAST({$wpdb->prefix}{$post_type}.infant_price AS DECIMAL) - CAST({$wpdb->prefix}{$post_type}.discount AS DECIMAL)
                                            ELSE
    ...
    {
        //print balancetags("<br><p class ='btn btn-primary' data-toggle='collapse' data-target='#st_cart_item".md5(json_encode($cart_item_key))."'>".__("Details" , 'traveler')."</p>");
        print balancetags('<br><span data-hide = "' . __("Less", 'traveler') . ' <i class=&quot;fa fa-angle-up&quot;>" data-more = "' . __("More", 'traveler') . ' <i class=&quot;fa fa-angle-down&quot;>" data-target= "#st_cart_item' . md5(json_encode($cart_item_key)) . '" data-toggle="collapse" class="_show_wc_cart_item_information_btn text-color booking-item-review-expand-new">' . __("More", 'traveler') . ' <i class="fa fa-angle-down"></i></span>');
    }
    ...
    public function checkSecurity($key = 'st_frontend_security')
    {
