...
if (!class_exists('TravelHelper')) {
    ...
    class TravelHelper {
        ...
        static function _get_price_avg_or_min($id_hotel) {

            global $wpdb;

            $table_st_hotel = $wpdb->prefix . 'st_hotel';

            $table_availability = $wpdb->prefix . 'st_room_availability';

            $hotel_room = $wpdb->prefix . 'hotel_room';
            ...
            $sql .= 'SELECT `parent_id` , min(CAST(CASE WHEN IFNULL(table_availability.adult_price, 0) = 0 THEN table_availability.price ELSE table_availability.adult_price END AS DECIMAL) - (CAST(CASE WHEN IFNULL(table_availability.adult_price, 0) = 0 THEN table_availability.price ELSE table_availability.adult_price END AS DECIMAL) * ((IFNULL(hotel_room.discount_rate, 1))/100))) as st_price, table_st_hotel.price_avg as st_price_avg FROM ' . $wpdb->prefix . 'posts as posts';

            $sql .= ' INNER JOIN ' . $table_st_hotel . ' as table_st_hotel ON posts.ID = table_st_hotel.post_id';

            $sql .= ' INNER JOIN ' . $table_availability . ' as table_availability ON  posts.ID = table_availability.parent_id';

            $sql .= ' INNER JOIN ' . $hotel_room . ' as hotel_room ON  posts.ID = hotel_room.room_parent';

            $sql .= ' WHERE posts.ID=' . (int) $id_hotel;
        ...
        static function init() {
            ...
            add_filter('wp_nav_menu', [__CLASS__, 'add_menu_below'], 10, 2);

            add_action('wp_insert_comment', [__CLASS__, 'comment_inserted'], 99, 2);



            add_filter('get_comments_number', [__CLASS__, '_reacount_comment_number'], 10, 2);

        }



        public static function _reacount_comment_number($count, $post_id) {

            $comment_count = get_comment_count($post_id);

            $count = (int) $comment_count['approved'];
        ...
        public static function comment_inserted($comment_id, $comment_object) {
        ...
        public static function add_menu_below($nav_menu, $args) {

            $menu_style = st()->get_option('menu_style_modern', "");

            if ($menu_style == '3' && !is_user_logged_in() && $args->theme_location == 'primary') {

                $nav_menu = substr($nav_menu, 0, -5);

                $nav_menu .= '<li class="topbar-item login-item">

                <a href="" class="login" data-toggle="modal"

                   data-target="#st-login-form">' . esc_html__('Login', 'traveler') . '</a>

            </li>' . '<li class="topbar-item signup-item">

                    <a href="" class="signup" data-toggle="modal"

                       data-target="#st-register-form">' . esc_html__('Sign Up', 'traveler') . '</a>
        ...
        static function change_google_fonts_api_key($api_key) {
            ...
            $option_api = st()->get_option('google_font_api_key', 'AIzaSyDzH_BKnGaGm4h4ZplIuZkJYU9fij-XaqU');
            ...
            if ($mapping_detect == 'on') {
                ...
                if ($wpml_options['automatic_redirect'] == 1 || $wpml_options['automatic_redirect'] == 2) {
                    ...
                    if (!empty($mapping_currency)) {
                        ...
                        if (isset($_SESSION['change_currencyds']) && $_SESSION['change_currencyds'] != 'ok') {
        ...
        static public function get_currency_by_language($language){
            ...
            if ($mapping_detect == 'on') {
                ...
                if ($wpml_options['automatic_redirect'] == 1 || $wpml_options['automatic_redirect'] == 2) {
                    ...
                    $mapping_currency = get_option('mapping_currency_' . $language);
        ...
        public static function pag_filter_author($query) {
            ...
            if (!is_admin() && $query->is_main_query()) {
                ...
                if ($query->is_author()) {
                    ...
                    $query->set('post_type', ['st_hotel', 'st_tours', 'st_activity', 'st_cars', 'st_rental', 'hotel_room']);
        ...
        static public function st_author_contact() {
            ...
            $subject = sprintf(__('[%s] Contact from customer', 'traveler'), $title);
            ...
            if ($data[0]['value'] == '' || $data[1]['value'] == '' || $data[2]['value'] == '' || $data[3]['value'] == '') {
                ...
                $res = [
                    ...
                    'message' => __("Please fill all required fields", 'traveler')
            ...
            if ($check) {
                ...
                $res = [
                    ...
                    'message' => __("Send email successfully!", 'traveler')
                ...
                $res = [
                    ...
                    'message' => __("Some errors occurred!", 'traveler')
        ...
        static function _add_css_vc() {
            ...
            if (strpos($slug, 'template-') == 0) {
                ...
                if (!empty($shortcodes_custom_css)) {
                    ...
                    echo balanceTags($shortcodes_custom_css);
        ...
        static function get_default_currency($need = false) {
            ...
            if ($primary_obj) {
                ...
                if ($need and isset($primary_obj[$need]))

                    return $primary_obj[$need];
                ...
                if (isset($all_currency[0])) {
                    ...
                    if ($need and isset($all_currency[0][$need]))

                        return $all_currency[0][$need];
        ...
        static function get_current_currency($need = false) {
            ...
            if (isset($_SESSION['currency']['name'])) {
                ...
                if ($session_currency = self::find_currency($name)) {
                    ...
                    if ($need and isset($session_currency[$need]))

                        return $session_currency[$need];
        ...
        static function format_money_from_db($money = '', $currency = false) {
            ...
            if (is_array($decimal_separator) || empty($decimal_separator)) {

                $decimal_separator = st()->get_option('decimal_separator', '.');

            }

            if (is_array($thousand_separator) || empty($thousand_separator)) {
        ...
        static function convertDateFormatNew($date) {
        ...
        static function getDateFormatMomentText() {
        ...
        static function paging($query = false, $wrapper = true) {
            ...
            $arg = [
                ...
                'prev_text' => __('Previous Page', 'traveler'),

                'next_text' => __('Next Page', 'traveler'),
        ...
        static function paging_room($query = false) {
            ...
            if ($paged > 1) {
                ...
                $html .= ' <li><a class="pagination paged_item_room" data-page="' . ($paged - 1) . '">' . __('Previous', 'traveler') . '</a></li>';
            ...
            if ($paged < $i - 1) {
                ...
                $html .= ' <li><a class="pagination paged_item_room" data-page="' . ($paged + 1) . '">' . __('Next', 'traveler') . '</a></li>';
        ...
        static function paging_single_partner($query = false, $user_id) {
            ...
            $html = '
                     ...
                     <div class="mt30">
                        ...
                        <span style="padding-left: 10px">' . __("Showing", 'traveler') . ' ' . ($posts_per_page * ($paged - 1) + 1) . '-' . $last . '</span>
            ...
            if ($paged > 1) {
                ...
                $html .= ' <li><a class="pagination paged_item_service" data-user-id="' . $user_id . '" data-post-type="' . $post_type . '" data-page="' . ($paged - 1) . '">' . __('Previous', 'traveler') . '</a></li>';
            ...
            if ($paged < $i - 1) {
                ...
                $html .= ' <li><a class="pagination paged_item_service" data-user-id="' . $user_id . '" data-post-type="' . $post_type . '" data-page="' . ($paged + 1) . '">' . __('Next', 'traveler') . '</a></li>';
        ...
        static function comments_list_new($comment, $args, $depth) {

            $file = locate_template('st_templates/layouts/modern/blog/comment-list.php');
        ...
        static function comments_list_new_single_hotel($comment, $args, $depth) {

            $file = locate_template('st_templates/layouts/modern/single_hotel/blog/comment-list.php');
        ...
        static function reviewlistnew() {

            $file = locate_template('st_templates/layouts/modern/common/reviews/review-list.php');
        ...
        static function st_get_attribute_advance($post_type) {
        ...
        static function getListFullNameLocationCar($id_car, $post_type = '') {

            if (array_key_exists('full_name_' . $post_type, self::$listFullNameLocations))

                return self::$listFullNameLocations['full_name_' . $post_type];
        ...
        static function getListFullNameLocationHotel() {



            global $wpdb;

            $sql = "SELECT nest.location_id as ID,nest.name  as location_name FROM {$wpdb->prefix}st_location_nested as nest

            INNER JOIN {$wpdb->prefix}st_location_relationships as relation ON nest.location_id = relation.location_from

            WHERE  relation.post_type = 'st_hotel' GROUP BY nest.location_id";
            ...
            if (!$number or $number == 1) {
                ...
                switch ($value) {
                    ...
                    case 'month':
                        ...
                        return __("month", 'traveler');

                        break;

                    case 'week':

                        return __("week", 'traveler');

                        break;

                    case 'hour':

                        return __("hour", 'traveler');

                        break;

                    default:

                        return __("day", 'traveler');
                ...
                switch ($value) {
                    ...
                    case 'month':
                        ...
                        return __("months", 'traveler');

                        break;

                    case 'week':

                        return __("weeks", 'traveler');
        ...
        static function get_opt_menus() {
            ...
            if (empty($menus) or ! is_array(wp_get_nav_menus()))
        ...
        static function isset_table($table_name) {
            ...
            if (!empty(self::$_check_table_duplicate[$table_name]))
            ...
            if (count($return)) {
                ...
                foreach ($return as $key => $item) {
                    ...
                    if ($key == 'st_hotel') {
                        ...
                        $name = count($item) > 1 ? __('Hotels', 'traveler') : __('Hotel', 'traveler');

                    }

                    if ($key == 'st_rental') {

                        $name = count($item) > 1 ? __('Rentals', 'traveler') : __('Rental', 'traveler');

                    }

                    if ($key == 'st_cars') {

                        $name = count($item) > 1 ? __('Cars', 'traveler') : __('Car', 'traveler');

                    }

                    if ($key == 'st_tours') {

                        $name = count($item) > 1 ? __('Tours', 'traveler') : __('Tour', 'traveler');

                    }

                    if ($key == 'st_activity') {

                        $name = count($item) > 1 ? __('Activities', 'traveler') : __('Activity', 'traveler');

                    }

                    if ($key == 'hotel_room') {

                        $name = count($item) > 1 ? __('Hotel rooms', 'traveler') : __('Hotel room', 'traveler');

                    }

                    if ($key == 'rental_room') {

                        $name = count($item) > 1 ? __('Rental rooms', 'traveler') : __('Rental room', 'traveler');

                    }

                    if ($key == 'st_flight') {

                        $name = count($item) > 1 ? __('Flights', 'traveler') : __('Flight', 'traveler');

                    }

                    echo '<div style="margin-bottom: 5px;">';

                    echo sprintf(__('ALERT: We Have %d new fishing trip(s) that needs to be verified for approval.', 'traveler'), count($item), $name);

                    echo '<a style="margin-left: 5px;" href="' . admin_url('edit.php?post_status=draft&post_type=' . $key) . '" target="_blank">' . __('Click Here', 'traveler') . '!</a>';
        ...
        static function st_admin_notice_update_location() {
            ...
            $name = ($count > 1) ? __('Locations', 'traveler') : __('Location', 'traveler');
            ...
            if ($count >= 1):
                ...
                echo sprintf(__('Have %d %s need to update for google maps search.', 'traveler'), $count, $name);

                echo '<a style="margin-left: 5px;" href="' . admin_url('edit.php?post_type=location&st_update_glocation') . '" target="_blank">' . __('Click Here', 'traveler') . '!</a>';
        ...
        static function st_admin_notice_user_partner_check_approved() {
            ...
            if (count($data_user_register) > 0 or count($data_user_update_certificates) > 0) {
                ...
                if (count($data_user_register) > 0) {
                    ...
                    echo sprintf(__('ALERT:  There is %d potential Captain(s) waiting to be verified and approved!', 'traveler'), count($data_user_register));

                    echo '<a style="margin-left: 5px;" href="' . admin_url('admin.php?page=st-users-list-partner-menu&st_tab=partner_pending') . '" target="_blank">' . __('Click Here', 'traveler') . '!</a><br>';

                }

                if (count($data_user_update_certificates) > 0) {

                    echo sprintf(__('Have %d new user partner update certificates need check for approved.', 'traveler'), count($data_user_update_certificates));

                    echo '<a style="margin-left: 5px;" href="' . admin_url('admin.php?page=st-users-list-partner-menu&st_tab=partner_update') . '" target="_blank">' . __('Click Here', 'traveler') . '!</a><br>';
            ...
            switch ($post_type) {
                ...
                case "hotel_room":
                    ...
                    if (get_post_meta(get_the_ID(), 'st_custom_layout', true))
                        ...
                        $detail_layout = get_post_meta(get_the_ID(), 'st_custom_layout', true);
        ...
        static function st_approved_item($author, $post) {
            ...
            if (st()->get_option('enable_email_approved_item', 'off') == 'off')
                ...
                foreach ($terms as $key2 => $value2) {
                    ...
                    <div class="checkbox" style="margin-left: <?php echo (22 * $level) . 'px'; ?>">
                        ...
                        <label>
                            ...
                            <input <?php if ($checked) echo "checked"; ?>

                                value="<?php echo esc_attr($value2->term_id); ?>"
            ...
            if (!empty($terms)):
                ...
                foreach ($terms as $key2 => $value2) {
                    ...
                    <div style="margin-left: <?php echo (22 * $level) . 'px'; ?>"
                         ...
                         data-value="<?php echo esc_attr($value2->term_id); ?>"><span
        ...
        static function get_pages_travelpayouts_api() {
            ...
            $page_list[] = [
                ...
                'label' => esc_html__('-- Choose One --', 'traveler')
        ...
        static function post_translated_new($post_id, $post_type = 'post', $lang = '') {
        ...
        static function transferPostType() {
        ...
        static function transferDestinationOptionNew() {
        ...
        static function transferDestinationOptionNewFronend() {
        ...
        static function buildTreeOptionLocation($locations, $location_id) {
            ...
            if (is_array($locations) && count($locations)):
                ...
                foreach ($locations as $key => $value):
                    ...
                    <option data-style="padding-left: <?php echo esc_attr($level) . 'px;'; ?>" <?php selected($value['ID'], $location_id); ?>
                ...
                static function get_current_available_calendar($post_id) {
                    ...
                    if (!empty($post_id)) {
                        ...
                        if ($post_type == 'st_tours' || $post_type == 'st_activity') {
                            ...
                            $max_people = get_post_meta($post_id, 'rental_number', true);
            ...
            <div class="layzyload-wrapper">
                ...
                <img class="st-lazy" data-src="<?php echo esc_url($thumb_src); ?>"
        ...
        static function get_rate_review_text($review, $count = null) {
            ...
            $string = esc_html__('Excellent', 'traveler');
            ...
            if ($review > 4) {
                ...
                $string = esc_html__('Very Good', 'traveler');

            } elseif ($review > 2) {

                $string = esc_html__('Average', 'traveler');

            } elseif ($review > 1) {

                $string = esc_html__('Poor', 'traveler');

            } elseif ( $review > 0 && $review <= 1 ) {

                $string = esc_html__('Terrible', 'traveler');

            } else {

                $string = esc_html__('Not Rated', 'traveler');
        ...
        static function comment_form( $args = array(), $post_id = null ) {
            ...
            <div id="respond" class="comment-respond <?php echo esc_attr( $args['class_container'] ); ?>" data-toggle-section="st-review-form"
                ...
                echo $args['title_reply_after'];
                    ...
                    if (get_post_type($post_id) == 'hotel_room') {
                        ...
                        echo st()->load_template('layouts/modern/hotel/room_review_form');

                    } elseif (get_post_type($post_id) == 'st_tours') {

                        echo st()->load_template('layouts/modern/tour/elements/tour_review_form');

                    } elseif (get_post_type($post_id) == 'st_activity') {

                        echo st()->load_template('layouts/modern/activity/elements/tour_review_form');

                    } elseif (get_post_type($post_id) == 'st_hotel') {

                        echo st()->load_template('layouts/modern/hotel/hotel_review_form');

                    } elseif (get_post_type($post_id) == 'st_rental') {

                        echo st()->load_template('layouts/modern/rental/rental_review_form');

                    } elseif (get_post_type($post_id) == 'st_cars') {

                        echo st()->load_template('layouts/modern/car/car_review_form');
        ...
        static function comment_form_post($args = [], $post_id = null) {
            ...
            if($style == 2){
                ...
                $class_form_comment = 'st-comment-solo';
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                ...
                                <textarea name="comment" class="form-control has-matchHeight"
                                          ...
                                          placeholder="<?php echo __('Comment*', 'traveler') ?>"></textarea>

                            </div>

                            <div class="col-xs-12 col-sm-<?php echo esc_attr($col_small);?> <?php echo esc_attr($class_more);?>">

                                <input type="text" class="form-control" name="author"

                                       placeholder="<?php echo __('Name*', 'traveler') ?>">
                            ...
                            <div class="col-xs-12 col-sm-<?php echo esc_attr($col_small);?> <?php echo esc_attr($class_more);?>">
                                ...
                                <input type="email" class="form-control" name="email"
                                       ...
                                       placeholder="<?php echo __('Email*', 'traveler') ?>">
                            ...
                            <div class="col-xs-12 col-sm-4">
                                ...
                                <input type="url" class="form-control" name="url"
                                       ...
                                       placeholder="<?php echo __('Website', 'traveler') ?>">
        ...
        static function pagination_comment($args = []) {
            ...
            $defaults = [
                ...
                'previous_string' => __('Prev', 'traveler'),

                'next_string' => __('Next', 'traveler'),
            ...
            $args = wp_parse_args(
                    ...
                    $args, apply_filters('st_pagination_comment_defaults', $defaults)
            ...
            if (!$args['total']) {
                ...
                $args['total'] = (int) $comments_count->approved;
            ...
            $count = ceil($args['total'] / $comment_per_page);



            $page = intval(STInput::get('comment_page', 1));
            ...
            $previous = esc_url(add_query_arg(['comment_page' => $previous, '' => '#reviews']));

            if ($previous && (1 == $page)) {

                $echo .= '<li class="disabled"><a class="page-item" href="javascript:void(0);" title="' . esc_html__('previous', 'traveler') . '">' . balanceTags($args['previous_string']) . '</a></li>';

            }

            if ($previous && (1 != $page)) {

                $echo .= '<li class=""><a class="page-item" href="' . esc_url($previous) . '" title="' . esc_html__('previous', 'traveler') . '">' . balanceTags($args['previous_string']) . '</a></li>';
            ...
            if (!empty($min) && !empty($max)) {
                ...
                for ($i = $min; $i <= $max; $i++) {
                    ...
                    if ($page == $i) {
                        ...
                        $echo .= '<li class="active "><a class="current-page" href="javascript:void(0);">' . str_pad((int) $i, 1, '0', STR_PAD_LEFT) . '</a></li>';

                    } else {

                        $echo .= sprintf('<li class=""><a class="page-item" href="%s">%2d</a></li>', esc_url(add_query_arg(['comment_page' => $i, '' => '#reviews'])), $i);
            ...
            $next = esc_url(add_query_arg(['comment_page' => $next, '' => '#reviews']));

            if ($next && ($count == $page)) {

                $echo .= '<li class="disabled"><a class="page-item" href="javascript:void(0);" title="' . esc_html__('next', 'traveler') . '">' . balanceTags($args['next_string']) . '</a></li>';

            }

            if ($next && ($count != $page)) {

                $echo .= '<li class=""><a class="page-item" href="' . esc_url($next) . '" title="' . esc_html__('next', 'traveler') . '">' . balanceTags($args['next_string']) . '</a></li>';

            }

            if (isset($echo))

                echo balanceTags($args['before_output']) . balanceTags($echo) . balanceTags($args['after_output']);
        ...
        static function paging_in_location_single($query, $wrapper = true) {
        ...
        static function _update_rate_review($id, $rate_review = null, $post_type = 'st_hotel')

        {

            if (!TravelHelper::checkTableDuplicate($post_type)) return;



            $num_rows = TravelHelper::checkIssetPost($id, $post_type);
            ...
            if ($num_rows == 1) {
                ...
                TravelHelper::updateDuplicate($post_type, $data, $where);
