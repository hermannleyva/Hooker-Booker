				...
				for($i = $data['check_in']; $i <= $data['check_out']; $i = strtotime('+1 day', $i)){
					...
					if(is_array($result) && count($result)){
						...
						if($number_room_cart + $number_room > $total_room){
							...
							$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> room has only %s room(s) available on <b>%s</b>.','traveler'), get_the_title($data['room_id']), $free_room, date(TravelHelper::getDateFormat(),$i)).'</p>';
							return false;
						}
					}else{
						if($number_room_cart > $total_room){
							$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> room has only %s room(s) available on <b>%s</b>.','traveler'), get_the_title($data['room_id']), ($total_room), date(TravelHelper::getDateFormat(),$i)).'</p>';
							return false;
						}
					}
				}
				...
				for($i = $data['check_in']; $i <= $data['check_out']; $i = strtotime('+1 day', $i)){
					...
					if(is_array($result) && count($result)){
						...
						if($number_room_cart + $number_room > $total_rental){
							...
							$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> rental has only %s rental(s) available on <b>%s</b>.','traveler'), get_the_title($data['rental_id']), ($free_room), date(TravelHelper::getDateFormat(),$i)).'</p>';
							return false;
						}
					}else{
						if($number_room_cart > $total_rental){
							$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> rental has only %s rental(s) available on <b>%s</b>.','traveler'), get_the_title($data['rental_id']), ($total_rental), date(TravelHelper::getDateFormat(),$i)).'</p>';
							return false;
						}
					}
				}
