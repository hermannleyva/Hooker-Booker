		...
		static function get_cart_data($cart, $post_type){
			...
			if($post_type == 'st_hotel' && count($cart)){
				foreach($cart as $key => $val){
					if($val['st_booking_data'][''] == 'st_hotel'){
			...
			if($post_type == 'hotel_room' && count($cart)){
				foreach($cart as $key => $val){
					if($val['st_booking_data'][''] == 'hotel_room'){
					...
					for($i = intval($val['min']); $i <= intval($val['max']); $i = strtotime('+1 day', $i) ){
						...
						if(is_array($result) && count($result)){
							...
							if($number_room_cart + $number_room > $total_room){
								...
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> room has only %s room(s) available on <b>%s</b>.','traveler'), get_the_title($room_id), $free_room, date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}else{
							$number_room_cart = 0;
							...
							if($number_room_cart > $total_room){
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> room has only %s room(s) available on <b>%s</b>.','traveler'), get_the_title($room_id), ($total_room), date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}
					}
					...
					for($i = intval($val['min']); $i <= intval($val['max']); $i = strtotime('+1 day', $i) ){
						...
						if(is_array($result) && count($result)){
							...
							if($number_room_cart + $number_room > $total_room){
								...
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> rental has only %s rental(s) available on <b>%s</b>.','traveler'), get_the_title($rental_id), $free_room, date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}else{
							$number_room_cart = 0;
							...
							if($number_room_cart > $total_room){
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> rental has only %s rental(s) available on <b>%s</b>.','traveler'), get_the_title($rental_id), ($total_room), date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}
					}
						...
						if(is_array($result) && count($result)){
							...
							if( $max_people > 0 ){
								if($people_cart + $people > $max_people){
									...
									$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> tour has only %s people available on <b>%s</b>.','traveler'), get_the_title($tour_id), $free_people, date(TravelHelper::getDateFormat(),$i)).'</p>';
									return false;
								}
							}
							...
							if( $max_people > 0 ){
								if($people_cart > $max_people){
									$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> tour has only %s people available on <b>%s</b>.','traveler'), get_the_title($tour_id), ($max_people), date(TravelHelper::getDateFormat(),$i)).'</p>';
									return false;
								}
							}
					...
					for($i = intval($val['min']); $i <= intval($val['max']); $i = strtotime('+1 day', $i) ){
						...
						if(is_array($result) && count($result)){
							...
							if($people_cart + $people > $max_people){
								...
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> activity has only %s people(s) available on <b>%s</b>.','traveler'), get_the_title($activity_id), $free_people, date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}else{
							$people_cart = 0;
                            ...
                            if( $max_people > 0 ){
                                if($people_cart > $max_people){
                                    $_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> activity has only %s people(s) available on <b>%s</b>.','traveler'), get_the_title($activity_id), ($max_people), date(TravelHelper::getDateFormat(),$i)).'</p>';
                                    return false;
                                }
                            }
						} 
					...
					for($i = intval($val['min']); $i <= intval($val['max']); $i = strtotime('+1 day', $i) ){
						...
						if(is_array($result) && count($result)){
							...
							if($number_car_cart + $number_car > $max_car){
								...
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('The <b>%s</b> car has only %s item(s) available on <b>%s</b>.','traveler'), get_the_title($car_id), $free_car, date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						}else{
							$number_car_cart = 0;
							...
							if($number_car_cart > $max_car){
								$_SESSION['flash_validate_checkout'] = '<p class="bg-danger" style="padding: 10px 5px">'.sprintf(__('This <b>%s</b> car has only %s item(s) available on <b>%s</b>.','traveler'), get_the_title($car_id), ($max_car), date(TravelHelper::getDateFormat(),$i)).'</p>';
								return false;
							}
						} 
