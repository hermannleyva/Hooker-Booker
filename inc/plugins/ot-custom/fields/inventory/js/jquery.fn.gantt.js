...
(function ($) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings  = {
            ...
            months          : locale_daterangepicker.monthNames,

            dow             : locale_daterangepicker.daysOfWeek,

            startPos        : new Date(),

            navigate        : locale_daterangepicker.buttons,
            ...
            waitText        : st_params.please_waite,

            zoomBar         : false,//=== custom by shinetheme
        ...
        var core = {
            ...
            init: function (element) {
                ...
                core.clickStart = null;

                core.clickEnd   = null;

                core.flagClick  = false;
                ...
                try {
                    ...
                    //==== custom by shinetheme

                    if (settings.dateStart != '') {

                        element.dateStart = new Date(settings.dateStart);
                    ...
                    if (settings.dateEnd != '') {
                        ...
                        element.dateEnd = new Date(settings.dateEnd);
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        entries.push('<span class="fn-label' + (entry.cssClass ? ' ' + entry.cssClass : '') + '" data-id="' + entry.id + '" data-price-per-person="' + entry.price_by_per_person + '">' + (entry.name + '<div class="inventory-edit-room-number dashicons dashicons-edit"></div>' || '') + '</span>');
            ...
            dataPanel: function (element, width) {
                ...
                dataPanel.click(function (e) {
                    ...
                    if ($(e.target).parent().is('.day') || $(e.target).parent().is('.date') || $(e.target).parent().is('.header')) {
                    ...
                    var col = core.elementFromPoint(e.pageX, datapanel.offset().top + 80);
                    ...
                    var price_by_per_person = (row.find('.fn-label').data('price-per-person')) ? row.find('.fn-label').data('price-per-person') : false;
                    ...
                    if ($('.gantt-range', datapanel).length == 0) {

                        var range = '<div class="gantt-range"></div>';

                        datapanel.append(range);

                    }

                    if ((core.clickStart && !core.flagClick && core.clickEnd)) {

                        $('.gantt-range', datapanel).remove();
                        ...
                        $('.panel-price-wrapper', element).remove();

                        return datapanel;

                    }

                    if ((core.clickStart == null)) {

                        $('.gantt-range', datapanel).css({

                            top  : row_offset + 1,

                            width: 102,

                            left : parseInt(col.attr('offset'))

                        }).removeClass('selected').addClass('range-moving');
                    ...
                    if (core.clickStart && core.flagClick && core.clickEnd == null) {

                        var old_left = parseInt($('.gantt-range', datapanel).css('left').replace('px', ''));

                        $('.gantt-range', datapanel).css({

                            width: parseInt(col.attr('offset')) - old_left + 102

                        }).addClass('selected').removeClass('range-moving');
                        ...
                        if ($('.panel-price-wrapper', element).length == 0) {

                            if (price_by_per_person)

                                var str_price_wrap = '<div class="panel-price-wrapper"><div class="panel-price"><input class="input-price" type="text" name="input-adult-price" autofocus value="" placeholder="'+ st_params.text_adult_price +'"><input class="input-price" type="text" name="input-child-price" value="" placeholder="'+ st_params.text_child_price +'"><select class="input-status" name="input-status"><option value="available">'+ st_params.text_available +'</option><option value="unavailable">'+ st_params.text_unavailable +'</option></select><a href="javascript: void(0);" class="button wb-btn-add-price" style="margin-left: 10px;">'+ st_params.text_update +'<i class="fa fa-spin fa-spinner loading-icon"></i></a><span class="close"><i class="fa fa-times"></i></span></div></div>';

                            else

                                var str_price_wrap = '<div class="panel-price-wrapper"><div class="panel-price"><input class="input-price" type="text" name="input-price" value="" placeholder="'+ st_params.text_price +'"><select class="input-status" name="input-status"><option value="available">'+ st_params.text_available +'</option><option value="unavailable">'+ st_params.text_unavailable +'</option></select><a href="javascript: void(0);" class="button wb-btn-add-price" style="margin-left: 10px;">'+ st_params.text_update +'<i class="fa fa-spin fa-spinner loading-icon"></i></a><span class="close"><i class="fa fa-times"></i></span></div></div>';

                            $(element).append(str_price_wrap);

                            $('.input-price', element).focus();

                            $('.close', '.panel-price-wrapper').click(function () {
                            ...
                            $('.wb-btn-add-price', '.panel-price-wrapper').click(function (e) {
                                ...
                                var data = {
                                    ...
                                    'adult_price': $('.input-price[name=input-adult-price]', '.panel-price-wrapper').val() || '',

                                    'child_price': $('.input-price[name=input-child-price]', '.panel-price-wrapper').val() || ''

                                };

                                var t    = $(this);

                                $('.loading-icon', t).fadeIn();
                                ...
                                $.post(ajaxurl, data, function (respon) {
                                    ...
                                    if (typeof respon == 'object') {
                                        ...
                                        if (respon.status == 0) {
                                            ...
                                            $(element).trigger('wpbooking_update_price_inventory', [settings.dateStart, settings.dateEnd]);

                                        }

                                    }

                                    $('.loading-icon', t).fadeOut();
                    ...
                    settings.onAddClick(dt, row_offset, col);
                ...
                switch (settings.scale) {
                    ...
                    case "hours":
                        ...
                        for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            horArr.push('<div class="row day ' +
                                ...
                                '"  offset="' + i * 102 + '"  repdate="' + rday.getRepDate() + '"><div class="fn-label">' +
                    ...
                    case "weeks":
                        ...
                        for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            dayArr.push('<div class="row day wd" ' +
                                ...
                                ' id="' + rday.getWeekId() + '" offset="' + i * 102 + '" repdate="' + rday.getRepDate() + '"> ' +
                    ...
                    case 'months':
                        ...
                        for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            monthArr.push('<div class="row day wd" id="dh-' + tools.genId(rday.getTime()) + '" offset="' + i * 102 + '" repdate="' + rday.getRepDate() + '">' + (1 + rday.getMonth()) + '</div>');
                    ...
                    default:
                        ...
                        for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            dayArr.push('<div class="row date ' + day_class + '" ' +
                                ...
                                ' id="dh-' + tools.genId(rday.getTime()) + '" offset="' + i * 102 + '" repdate="' + rday.getRepDate() + '"> ' +
                        ...
                        yearArr.push(
                            ...
                            102 * daysInYear + 'px;"><div class="fn-label">' +
                        ...
                        monthArr.push(
                            ...
                            102 * daysInMonth + 'px"><div class="fn-label">' +

                            settings.months[month] +

                            '</div></div>');

                        var dataPanel = core.dataPanel(element, range.length * 102 + 1);
            ...
            navigation: function (element) {
                ...
                if (settings.navigate === "scroll") {
                    ...
                    ganttNavigate = $('<div class="navigate" />')
                        ...
                        .append($('<div class="nav-slider" />')
                            ...
                            .append($('<div class="nav-slider-left" />')
                                ...
                                .append($('<button type="button" class="nav-link nav-now"/>')
                                    ...
                                    .click(function () {
                                        ...
                                        $(element).trigger('wpbooking_now_inventory', [settings.dateStart, settings.dateEnd]);
                            ...
                            .append($('<div class="nav-slider-right" />')
                                ...
                                .append($('<button type="button" class="nav-link-custom nav-prev-month"/>')

                                    .html(st_params.prev_month)

                                    .click(function () {

                                        $(element).trigger('wpbooking_prev_month_inventory', [settings.dateStart, settings.dateEnd]);

                                    }))

                                .append($('<button type="button" class="nav-link-custom nav-next-month"/>')

                                    .html(st_params.next_month)

                                    .click(function () {

                                        $(element).trigger('wpbooking_next_month_inventory', [settings.dateStart, settings.dateEnd]);

                                    }))

                                .append($('<button type="button" class="nav-link-custom nav-guide"/>')

                                    .html('?')

                                    .click(function () {

                                        if ($('.inventory-guide', element).length == 0) {

                                            $(element).append('<div class="inventory-guide-wrapper"><div class="inventoty-guide">' +
                                                gantt.notice_guide
                                               +

                                                '<a href="javascript:void(0);" class="close">x</a>' +

                                                '</div></div>').addClass('showing');



                                            $(element).off('click', '.inventoty-guide .close').on('click', '.inventoty-guide .close', function () {

                                                $('.inventory-guide-wrapper', element).remove();
                    ...
                    if (settings.zoomBar) {//==== custom by shinetheme

                        ganttNavigate.append($('<button type="button" class="nav-link nav-zoomIn"/>')
            ...
            createProgressBar: function (id, days, cls, desc, label, dataObj, price, price_by_per_person = false, adult_price = '', child_price = '') {

                var cellWidth = tools.getCellSize();

                var barMarg   = tools.getProgressBarMargin() || 0;

                var str_bar = (price_by_per_person) ?

                                        '<div class="bar" data-price-per-person="'+ price_by_per_person +'"><div class="fn-label">' + label + '<div class="price">'+ st_params.text_adult + adult_price + '</div><div class="price">'+ st_params.text_child + child_price + '</div>' + '</div></div>'
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
                                    ...
                                    var top = tools.getCellSize() * 5 + parseInt(topEl.attr("offset"), 10);
                                ...
                                case "weeks":
                                    ...
                                    var top = tools.getCellSize() * 3 + parseInt(topEl.attr("offset"), 10);
