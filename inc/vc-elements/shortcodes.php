...
if (!function_exists('st_vc_blog')) {
    ...
    function st_vc_blog($attr, $content = false) {
        ...
        if(!empty($st_blog_number_post)){
            $query['posts_per_page'] = $st_blog_number_post;
        }
        if(!empty($st_blog_orderby)){
            $query['orderby'] = $st_blog_orderby;
        }
        ...
        while ($the_query->have_posts()) {
            ...
            if ($st_style == 'style4') {
                $txt .= st()->load_template('vc-elements/st-blog/loop4', false, $data);
            }
            if ($st_style == 'style5') {
                $txt .= st()->load_template('vc-elements/st-blog/loop5', false, $data);
            }
            if ($st_style == 'style6') {
                $txt .= st()->load_template('vc-elements/st-blog/loop6', false, $data);
            }
            if ($st_style == 'style7') {
                $txt .= st()->load_template('vc-elements/st-blog/loop7', false, $data);
                
            }
        }
        wp_reset_postdata();
        $r = '<div class="row row-wrap st_blog st_grid no_margin_inner ' . $st_style . ' ">
                 ' . $txt . '
               </div>';
        return $r;
    }
    ...
    function st_list_partner($arg) {
        ...
        if (!empty($results)) {
            ...
            foreach ($results as $key => $val) {
                ...
                $item_string = ($val->services <= 1) ? __(' service', 'traveler') : __(' services', 'traveler');
...
if (!function_exists('st_vc_rating_count')) {
    ...
    function st_vc_rating_count($attr) {
        ...
        if ($avg <= 4) {
            $return .= __('Good', 'traveler');
        } elseif ($avg <= 5) {
            $return .= __('Best', 'traveler');
        }
        $return .= " " . esc_attr($avg * 2) . "<small>/10</small>";
        $return .= "</b><br>";
        $reviews = STReview::count_comment(get_the_ID(), 'st_reviews');
        if ($reviews == 1)
            $return .= "<i>" . sprintf("from %s review", $reviews, 'traveler') . "</i>";
        if ($reviews > 1)
            $return .= "<i>" . sprintf("from %s reviews", $reviews, 'traveler') . "</i>";
...
if (!function_exists('st_search_form_func')) {
    ...
    function st_search_form_func($arg, $content = null) {
        $data = shortcode_atts(array(
            'st_title_form' => '',
            'st_post_type' => "st_hotel",
            'st_button_search' => __("Search", 'traveler')
                ), $arg, 'st_search_form');
        extract($data);
...
if (!function_exists('st_vc_slide_location')) {
    ...
    function st_vc_slide_location($attr, $content = false) {
        ...
        $data = shortcode_atts(
            ...
            'link_to' => 'page_search',
            'st_location_type' => '',
            'title' => __("Find Your Perfect Trip", 'traveler'),
            'opacity' => '0.5',
            'tabs_search' => 'all'
    ...
    function st_vc_team($attr, $content = false) {
        ...
        switch ($st_style) {
            ...
            case "st_tour_ver":
                ...
                $list_social .= "</div>";
                $txt .= '<div class="st_team_item_' . $st_style . '">
...
if (st_check_service_available('st_cars')) {
    ...
    if (!function_exists('st_search_cars_title')) {
        ...
        function st_search_cars_title($arg = array()) {
            ...
            if ($search_modal) {
                $html .= '<small><a class="popup-text" href="#search-dialog" data-effect="mfp-zoom-out">' . __('Change search', 'traveler') . '</a></small>';
            }
            $html .= '</h3>';
    ...
    if (!function_exists('st_vc_list_cars')) {
        ...
        function st_vc_list_cars($attr, $content = false) {
            ...
            if (!empty($st_search_args['st_ids'])) {
                $query['post__in'] = explode(',', $st_search_args['st_ids']);
...
if (st_check_service_available('st_rental')) {
    ...
    if (!function_exists('st_search_rental_title')) {
        ...
        function st_search_rental_title($arg = array()) {
            ...
            if ($search_modal) {
                $a .= '<small><a class="popup-text" href="#search-dialog" data-effect="mfp-zoom-out">' . __('Change search', 'traveler') . '</a></small>';
            }
            $a .= '</h3>';
    ...
    if (!function_exists('st_vc_list_tour')) {
        ...
        function st_vc_list_tour($attr, $content = false) {
            ...
            if (!empty($st_search_args['st_orderby']) and $st_orderby = $st_search_args['st_orderby']) {
                ...
                if ($st_orderby == 'discount') {
                    $query['order'] = $st_search_args['st_order'];
