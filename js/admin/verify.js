jQuery(function($){
    ...
    $('.verify-view').on('click', function(e){
    ...
    $('#user-verify-detail .close').on('click', function () {
    ...
    $(document).on('click', '.verify-box .btn-verify-single', function (e){
        ...
        var CLnotes = document.getElementById("CLdeny_rsn").value; 

        var BInotes = document.getElementById("BIdeny_rsn").value;
            ...
            $.ajax({
                ...
                complete  : function (res) {
                    ...
                    if(dataRes.status){
                        ...
                        if (dataRes.verify_info == 'review') {

                            vClass ='apart';

                            vText ='Pending Review';
                        }

                        if (dataRes.verify_info == 'resub') {
                            vClass ='none';
                            vText = 'Captain needs to Resubmit';
                        }
                        ...
                        $('#user-' + user_id).find('.verify-status').removeClass('all').removeClass('apart').removeClass('none').removeClass('half').addClass(vClass).text(vText);

                        $('.verify-box .verify-status').removeClass('all').removeClass('apart').removeClass('none').removeClass('half').addClass(vClass).text(vText);



                        if (criteria === '_deny') {

                            $(".captainLicenseStatus").text('Captain Needs to Review');

                            me.parent().find('.verify-status-item').removeClass('all').addClass('none');

                            me.parent().find('.optimaCL').removeClass('dashicons-yes').removeClass('dashicons-warning').addClass('dashicons-no');

                        } else if (criteria === '_denyBI') {

                            $(".boatInsuranceStatus").text('Captain Needs to Review');
                            ...
                            if (criteria === 'passport') {
                                ...
                                $(".captainLicenseStatus").text('Verified');

                                me.parent().find('.optimaCL').removeClass('dashicons-no').removeClass('dashicons-warning').addClass('dashicons-yes');

                            } else if (criteria === 'travel_certificate') {

                                me.parent().find('.verify-status-item').removeClass('all').removeClass('none').addClass('all');

                                $(".boatInsuranceStatus").text('Verified');

                                me.parent().find('.optimaBI').removeClass('dashicons-no').removeClass('dashicons-warning').addClass('dashicons-yes');
                            }
    ...
    $(document).on('click', '.verify-box .btn-verify-invalid', function (e){
        ...
        if(returnVal(criteria) == 0){
            ...
            me.parent().find('.invalid-reason').trigger('focus');
    ...
    $('#user-verify-detail').on('click', function (e) {
