...
<form class="booking-email-form" action="" method="post">
    ...
    <div class="form-group list-car-type">
        <a href="<?php echo add_query_arg(array('car_type' => 'normal', 'scaction' => 'email-notification'), TravelHelper::get_user_dashboared_link($page_link, 'booking-cars')) ?>" class="<?php if($st_tab=="normal") echo "current"; ?>"><i class="fa fa-car"></i> <?php echo __('Car normal', 'traveler'); ?></a>&nbsp; &nbsp; &nbsp;
        <a href="<?php echo add_query_arg(array('car_type' => 'transfer', 'scaction' => 'email-notification'), TravelHelper::get_user_dashboared_link($page_link, 'booking-cars')) ?>" class="<?php if($st_tab=="transfer") echo "current"; ?>"><i class="fa fa-tachometer"></i> <?php echo __('Car transfer', 'traveler'); ?></a>
    </div>
    ...
    <table class="table table-bordered table-striped table-booking-history table-booking-history-email">
        ...
        <?php if (!empty($posts)) {
                ...
                <tr>
                    ...
                    <td class="booking-history-type" style="text-align: left">
                        ...
                        if ($post_id) {
                            ...
                            echo '<a href="mailto:' . sanitize_email(get_post_meta($post_id, 'st_email', true)) . '">' . esc_html(sanitize_email(get_post_meta($post_id, 'st_email', true))) . '</a>';
            ...
            echo '<h5>' . __('No Cars', 'traveler') . '</h5>';
        }
        ?>
        </tbody>
    </table>
