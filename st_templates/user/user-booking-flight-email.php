...
<form class="form-inline booking-email-filter"
    ...
    <input type="hidden" name="sc" value="booking-flight"/>
    ...
    <div class="form-group">
        ...
        <input type="number" min="0" value="<?php echo esc_attr($from); ?>" name="from" class="form-control input-sm" id="out_of_from"
        ...
        <?php if (!empty($posts)) {
            ...
            foreach ($posts as $key => $value) {
                ...
                <tr>
                    ...
                    <td class=""> <?php
                        if ($post_id) {
                            ...
                            if (!empty($depart_data_location['origin_location_full']) && !empty($depart_data_location['destination_location_full'])) {
                                $title .= esc_html($depart_data_location['origin_location_full']) . ' (' . esc_html($depart_data_location['origin_iata']) . ') <i class="fa fa-long-arrow-right"></i> ' . esc_html($depart_data_location['destination_location_full']) . ' (' . esc_html($depart_data_location['destination_iata']) . ')';
                            }
                            if ($title) {
                    ...
                    $countdown_string = '<span class="number-countdown">' . esc_attr($date_diff) . '</span> ' . esc_html('day(s)', 'traveler');
                    ...
                    <td class="column-countdown <?php echo esc_attr($class_notice); ?>">
                        <?php echo esc_attr($countdown_string); ?><br />
                        <?php
                        if ($value->log_mail == '') {
                            ?>
            ...
            echo '<h5>' . __('No Tours', 'traveler') . '</h5>';
        }
        ?>
        </tbody>
    </table>
