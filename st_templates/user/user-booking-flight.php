...
?>
<div class="st-create">
    <h2><?php _e("Flight Booking", 'traveler') ?></h2>
    <?php
    $arr_query_arg = array(
        'sc' => 'booking-flight',
...
if (STInput::get('scaction') != 'email-notification') {
    $data_post = STUser_f::get_history_bookings('st_flight', $offset, $limit, $data->ID);
} else {
    $data_post = STUser_f::get_history_bookings_send_mail('st_flight', $offset, $limit, $data->ID);
}
$posts = $data_post['rows'];
$total = ceil($data_post['total'] / $limit);
if (STInput::get('scaction') == 'email-notification') {
    echo st()->load_template('user/user-booking-flight', 'email', array('posts' => $posts, 'offset' => $offset));
} else {
    ?>
<div class="infor-st-setting">
    <table class="table table-bordered table-striped table-booking-history">
        ...
        <tr>
            ...
            <th><?php _e("Flight Info", 'traveler') ?></th>
            <th class="hidden-xs"><?php _e("Execution Time", 'traveler') ?></th>
            <th class="hidden-xs" width="15%"><?php _e("Price", 'traveler') ?></th>
        ...
        <?php if (!empty($posts)) {
            ...
            foreach ($posts as $key => $value) {
                ...
                ?>
                <tr>
                    <td class="hidden-xs"><?php echo esc_attr($value->order_item_id); ?></td>
                    ...
                    <td class=""> <?php
                        if ($post_id) {
                            $title = '';
                            $depart_data_location = get_post_meta($post_id, 'depart_data_location', true);
                            if (!empty($depart_data_location['origin_location_full']) && !empty($depart_data_location['destination_location_full'])) {
                                $title .= $depart_data_location['origin_location_full'] . ' (' . $depart_data_location['origin_iata'] . ') <i class="fa fa-long-arrow-right"></i> ' . $depart_data_location['destination_location_full'] . ' (' . $depart_data_location['destination_iata'] . ')';
                            }
                            if ($title) {
                                echo "<p>" . balanceTags($title) . "</p>";
                            }
                        }
                        ?>
                    </td>
                    <td class="hidden-xs">
                        <?php
                        $date = $value->check_in;
                        if ($date) echo esc_html__('Depart Date: ', 'traveler') . date('d/m/Y', strtotime($date)); ?>
                        <br>
                        <?php $date = $value->check_out;
                        if ($date) echo esc_html__('Return Date: ', 'traveler') . date('d/m/Y', strtotime($date)); ?>
                    </td>
                    ...
                    <td class="">
                        ...
                        if($status == 'incomplete' || $status == 'pending' || $status == 'wc-processing' || $status == 'wc-on-hold' ){
                            ?>
            ...
            echo '<h5>' . __('No Tour', 'traveler') . '</h5>';
        }
        ?>
        </tbody>
    </table>
