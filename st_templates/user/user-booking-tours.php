...
?>
<div class="st-create">
    <h2><?php _e("Tours Booking", 'traveler') ?></h2>
    ...
    ?>
    <a class="st_button_send_mail" href="<?php echo add_query_arg($arr_query_arg, get_permalink()); ?>" class="btn btn-primary btn-sm btn-sendmail-notice-link" title="<?php echo __('Send email notification depart date', 'traveler'); ?>"><?php echo __('Send email notification', 'traveler'); ?></a>
    <?php } ?>
...
if(STInput::get('scaction') != 'email-notification') {
    ...
    $data_post = STUser_f::get_history_bookings_send_mail('st_tours', $offset, $limit, $data->ID);
}
$posts = $data_post['rows'];
$total = ceil($data_post['total'] / $limit);

if(STInput::get('scaction') == 'email-notification'){
    echo st()->load_template('user/user-booking-tours', 'email', array('posts' => $posts, 'offset' => $offset));
}else{
?>
...
<table class="table table-bordered table-striped table-booking-history">
    ...
    <tr>
        ...
        <th><?php _e("Tour Name", 'traveler') ?></th>
