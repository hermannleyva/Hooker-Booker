    ...
    if($car_by_location == 'off') {
		...
		<div class="media">
		  ...
		  <div class="media-body">
		    <h4 class="media-heading">' . esc_html($point['name']) . '</h4>
		    <p class="text-muted">' . esc_html($point['address']) . '</p>
		  </div>
		</div>
		';
		$content_tooltip = $point['name'].' - '.$point['address'];
		    ?>
            <option data-tooltip = "<?php echo esc_attr($content_tooltip);?>" data-tokens="<?php echo esc_attr( strtolower( $point['name'] ) ); ?>"
		...
		$content_tooltip = $point['name'].' - '.$point['address'];
		    ?>
            <option data-tooltip = "<?php echo esc_attr($content_tooltip);?>" data-tokens="<?php echo esc_attr(strtolower($point['name'])); ?>" data-content="<?php echo esc_attr($content); ?>" value="<?php echo esc_attr($point['id']); ?>" <?php selected($old_data, $point['id']) ?>>
			    <?php echo esc_attr($point['name']); ?>
            </option>
		    <?php
	    endforeach;
