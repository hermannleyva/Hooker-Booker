...
?>
<!-- start loop rental -->
<div <?php post_class('col-md-'.esc_attr($col).' style_box st_lazy_load')?> >
    ...
    <div class="thumb">
        ...
        <header class="thumb-header">
            <a class="hover-img" href="<?php the_permalink()?>">
                ...
                ?>
                <h5 class="hover-title-center"><?php _e('Book Now','traveler')?></h5>
            </a>
        ...
        <div class="thumb-caption">
            ...
            if(!empty($features)):?>
                ...
                foreach($features as $key=>$value):
                    ...
                    if($value['number']){
                        echo '<span class="booking-item-feature-sign">x '.esc_attr($value['number']).'</span>';
                    }
            ...
            <div>
                ...
                <?php
                if(!empty($price)){
                    echo '<span class="text-darken mb0 text-color">'.TravelHelper::format_money($price_sale).'<small> /'.__('night','traveler').'</small></span>';
                }
                ?>
            </div>
        </div>
